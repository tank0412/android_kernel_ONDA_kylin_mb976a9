			      ...
			      s64 timestamp)
{
	...
	memcpy((void*) &packet_size, &data[packet_size_offset], sizeof(packet_size));
			   ...
			   size_t n, char __user *buf)
{
	...
	/*find data size*/
	datasize = packet_size - sizeof(header);

	if(unlikely(n < datasize)) {
		INVSENS_LOGE("%s : size over = %x\n",
			__func__, datasize);
		kfifo_reset_out(&kf->kf);
		return -EFAULT;
	}
	n = rounddown(n, datasize);
